...
export async function GET(request: NextRequest) {
  ...
  const [ingresosRes, egresosRes, stateRes, clientesRes, proposalsRes, invoicesRes] =
    await Promise.all([
  ...
  const ingresos = (ingresosRes.data ?? []) as {
    persona?: string;
    usd?: number;
    period_month?: string;
    fecha?: string;
  }[];
  const egresos = (egresosRes.data ?? []) as {
    usd?: number;
    period_month?: string;
    fecha?: string;
  }[];
  const pendingPayments: PendingPayment[] =
    ((stateRes.data?.[0]?.payload) as PendingPayment[]) ?? [];
  ...
  const ingresosMes = ingresos.filter((r) =>
    (r.period_month || r.fecha || "").startsWith(currentMonth)
  );
  const cobradoMes = ingresosMes.reduce((a, r) => a + (r.usd || 0), 0);
  const egresosMes = egresos
    .filter((r) => (r.period_month || r.fecha || "").startsWith(currentMonth))
  ...
  const clientesConIngreso = new Set(
    ingresosMes.map((r) => r.persona?.toLowerCase().trim()).filter(Boolean)
  );

  const pendingMes = pendingPayments.filter(
  ...
  const invoicesMesPendientes = invoices.filter(
    (i) =>
      (i.period_month || "").startsWith(currentMonth) &&
  ...
  const cantPendientesMes =
    pendingMes.length + mensualidadesAuto.length + invoicesMesPendientes.length;

  const pendingTotalManual = pendingPayments
  ...
  const invoicesTotalPendientes = invoices
    .filter((i) => i.status === "pendiente" || i.status === "vencida")
    .reduce((a, i) => a + (i.monto_usd || 0), 0);
  ...
  return NextResponse.json({
    ...
    metrics: {
      ...
      cantPresupuestosMes,
      presupuestadoTotal,
      cantPresupuestosTotal,
      pendientesMes: pendientesMesTotal,
      cantPendientesMes,
      pendientesTotal,
    },
