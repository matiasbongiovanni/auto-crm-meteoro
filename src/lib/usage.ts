...
function emptyUsageOverview(reason: string): UsageOverview {
  return {
    snapshots: [],
    series: [],
    byModel: [],
...
async function resolveUsageAvailability() {
  if (LOCAL_USAGE_FLAG === "false")
    return {
      enabled: false,
      claudeAvailable: false,
      codexAvailable: false,
      reason: "Local usage ingestion is disabled by environment.",
    };
  const [claudeAvailable, codexAvailable] = await Promise.all([
    pathExists(CLAUDE_DIR),
    pathExists(CODEX_DB_PATH),
  ]);
  if (LOCAL_USAGE_FLAG === "true") {
    if (claudeAvailable || codexAvailable)
      return { enabled: true, claudeAvailable, codexAvailable, reason: null };
    return {
      enabled: false,
      claudeAvailable: false,
      codexAvailable: false,
      reason: "Local usage ingestion was enabled, but no local sources are available.",
    };
  }
  if (claudeAvailable || codexAvailable)
    return { enabled: true, claudeAvailable, codexAvailable, reason: null };
  return {
    ...
    claudeAvailable: false,
    codexAvailable: false,
    reason: "No local usage sources are available in this runtime.",
  };
}
      ...
      try {
        ...
        current.meta = {
          ...
          cache_creation_input_tokens:
            Number((current.meta?.cache_creation_input_tokens as number) || 0) + cacheCreation,
          cache_read_input_tokens:
            Number((current.meta?.cache_read_input_tokens as number) || 0) + cacheRead,
        };
...
print(json.dumps(rows))`;
  let stdout: string;
  try {
    ({ stdout } = await execFileAsync("python3", ["-c", python, CODEX_DB_PATH], {
  ...
  for (const row of rows) {
    ...
    current.meta = {
      ...(current.meta || {}),
      threads: Number((current.meta?.threads as number) || 0) + 1,
    };
    buckets.set(key, current);
  }
  return [...buckets.values()].sort((a, b) =>
...
function buildSeries(snapshots: UsageSnapshot[]): UsageSeriesPoint[] {
  ...
  for (const snapshot of snapshots) {
    const point = buckets.get(snapshot.period_start) || {
      date: snapshot.period_start,
      claude: 0,
      codex: 0,
    };
...
export async function getUsageOverview(): Promise<UsageOverview> {
  ...
  if (!availability.enabled)
    return emptyUsageOverview(
      availability.reason || "Local usage ingestion is unavailable."
    );
  const [claude, codex] = await Promise.all([
    availability.claudeAvailable ? readClaudeUsage() : Promise.resolve([]),
    availability.codexAvailable ? readCodexUsage() : Promise.resolve([]),
  ]);
  const snapshots = [...claude, ...codex].sort((a, b) =>
    a.period_start.localeCompare(b.period_start)
  );
  const summary = snapshots.reduce(
    (acc, item) => {
      if (item.source === "claude") {
        acc.claudeTokens += item.total_tokens;
        acc.claudeCostUsd = (acc.claudeCostUsd || 0) + (item.cost_usd || 0);
      } else {
        acc.codexTokens += item.total_tokens;
        acc.codexCostUsd = (acc.codexCostUsd || 0) + (item.cost_usd || 0);
      }
  ...
  return {
    snapshots,
    series: buildSeries(snapshots),
    byModel: buildModelBreakdown(snapshots),
    summary,
    ingestion: {
      claudeAvailable: availability.claudeAvailable,
